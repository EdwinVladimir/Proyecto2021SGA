...
export default {
  ...
  methods: {
    async listar() {
      const res = await axios.get('http://127.0.0.1:8000/api/modalidad');
      this.modalidad = res.data;
    },
    async eliminar(id) {
      const res = await axios.delete('http://127.0.0.1:8000/api/modalidad/' + id);
      this.listar();
    },
    async guardar() {
      if(this.modificar){
       
        const res = await axios.put('http://127.0.0.1:8000/api/modalidad/' + this.id, { 
        ...
        const res = await axios.post('http://127.0.0.1:8000/api/modalidad/', { 
